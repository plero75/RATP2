...
const DeparturePill: React.FC<{departure: any}> = ({ departure: dep }) => {
    ...
    let topContent = `${dep.remainingMinutes} min`;
    ...
    if (isImminent) {
        ...
        topContent = `${dep.remainingMinutes} min (+${dep.delayMinutes}')`;
    }
        ...
        <>
            <div className="space-y-2">
                {visitsByDestination.map(([destination, destVisits]) => {
                    const now = new Date();
                    const departures = destVisits.slice(0, 3).map(v => {
                        ...
                        <div key={destination} className="grid grid-cols-[1fr_auto] gap-2 items-center p-2 rounded-md bg-black/20">
                            ...
                            <div className="flex gap-1.5 items-center justify-end">
                                {departures.map((dep, index) => (
                                    <div key={dep.key} className="flex items-center gap-1">
                                        <DeparturePill departure={dep} />
                                        {index === 0 && dep.journeyRef && (
                                            <button 
                                                onClick={() => setSelectedJourney({ ref: dep.journeyRef, destination })}
                                                className="text-gray-400 hover:text-white transition-colors"
                                                aria-label="Détails" title="Détails"
                                            >
      ...
      {alertMessages.length > 0 && (
        <div className="flex flex-col gap-1 mb-2 p-2 bg-yellow-900/60 border border-yellow-700/80 rounded-lg">
          <div className="flex items-center gap-2 font-bold text-yellow-200 text-xs">
            <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M8.257 3.099c.636-1.21 2.242-1.21 2.878 0l6.363 12.182A1.75 1.75 0 0115.99 18H4.009a1.75 1.75 0 01-1.508-2.719L8.257 3.099zM10 6a.75.75 0 01.75.75v3.5a.75.75 0 01-1.5 0v-3.5A.75.75 0 0110 6zm0 8a1 1 0 100-2 1 1 0 000 2z" clipRule="evenodd" /></svg>
